...
MultipartFormStream.prototype._write = function(chunk, encoding, callback) {
    ...
    if (!this.inBoundaries) {
        ...
        if (startBoundary !== -1) {
            ...
            if (filename) {
                this.emit('file', filename, fieldname, this.fieldStream);
            } else {
            ...
            if (endBoundary !== -1) {
                this.fieldStream.push(chunk.slice(headerBoundary + 4, endBoundary));
            } else {
        ...
        if (endBoundary !== -1) {
            ...
            this.fieldStream.push(chunk.slice(0, endBoundary));
